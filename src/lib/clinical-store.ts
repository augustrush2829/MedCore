import 'server-only'
import type { AIResponse, ClinicalCase, LabResult, Patient, PatientLabResult } from '@/types'
...
export function analyzeCase(id: string) {
  const clinicalCase = getCase(id)
  if (!clinicalCase) return null
  const aiResponse: AIResponse = buildCaseAwareAIResponse(clinicalCase)
  return updateCase(id, { status: 'ai_complete', aiResponse })
}

function buildCaseAwareAIResponse(clinicalCase: ClinicalCase): AIResponse {
  const activeMedications = clinicalCase.medications.filter((medication) => medication.status === 'active')
  const abnormalLabs = clinicalCase.labResults.filter((lab) => lab.abnormalFlag)
  const labAttachments = (clinicalCase.attachments ?? []).filter((attachment) => attachment.section === 'labs')
  const hasLabEvidence = clinicalCase.labResults.length > 0 || labAttachments.length > 0
  const severeSymptoms = clinicalCase.symptoms.filter((symptom) => symptom.severity === 'severe')
  const symptomNames = clinicalCase.symptoms.map((symptom) => symptom.name).join(', ') || 'симптом оруулаагүй'
  const labSummary = abnormalLabs.length
    ? `Хэвийн бус лаборатори: ${abnormalLabs.map((lab) => `${lab.testName} ${lab.value} ${lab.unit}`).join(', ')}.`
    : clinicalCase.labResults.length
      ? 'Оруулсан лабораторийн үзүүлэлтүүдэд abnormal flag тэмдэглэгдээгүй.'
      : labAttachments.length
        ? `Лабораторийн ${labAttachments.length} зураг хавсаргасан, structured утга хараахан OCR/review хийгдээгүй.`
        : 'Лабораторийн мэдээлэл оруулаагүй.'
  const medicationSummary = activeMedications.length
    ? `Идэвхтэй эм: ${activeMedications.map((medication) => `${medication.name} ${medication.dose}`.trim()).join(', ')}.`
    : 'Одоогоор идэвхтэй хэрэглэж буй эм бүртгэгдээгүй.'

  const medicationWarnings: AIResponse['medicationWarnings'] = activeMedications
    .filter((medication) => /atorvastatin|statin/i.test(`${medication.name} ${medication.ingredients.join(' ')}`))
    .map((medication) => ({
      type: 'dose_risk',
      severity: 'high',
      description: `${medication.name} хэрэглэж байгаа тул элэгний фермент өссөн эсэхтэй хамт эмийн нөлөөг үнэлнэ. Энэ анхааруулга зөвхөн бүртгэлтэй эм дээр үндэслэсэн.`,
      medications: [`${medication.name} ${medication.dose}`.trim()],
    }))

  const chestPain = clinicalCase.symptoms.some((symptom) => /цээж|chest/i.test(symptom.name)) || /цээж|chest/i.test(clinicalCase.chiefComplaint)
  const redFlags = [
    ...(clinicalCase.hasRedFlag ? ['Энэ case улаан дохиотой гэж тэмдэглэгдсэн тул яаралтай эмчийн үнэлгээ шаардлагатай.'] : []),
    ...(chestPain ? ['Цээжний өвдөлт, амьсгал давчдах шинж байвал ЭКГ, тропонин болон яаралтай тусламжийн үнэлгээг хойшлуулахгүй.'] : []),
    ...severeSymptoms.map((symptom) => `${symptom.name} нь хүнд зэрэгтэй тэмдэглэгдсэн.`),
  ]

  const differentialDiagnosis = chestPain
    ? [
        {
          name: 'Цочмог титэм судасны хам шинжийг үгүйсгэх',
          confidence: 65,
          supportingEvidence: ['Цээжний өвдөлт/амьсгал давчдах зовиур', ...severeSymptoms.map((symptom) => `${symptom.name} хүнд зэрэгтэй`)],
          missingEvidence: ['ЭКГ', 'Тропонин', 'Цусны даралт, SpO2 зэрэг амин үзүүлэлт'],
          icdCode: 'I24',
        },
        {
          name: 'Уушгины гаралтай амьсгалын зовиур',
          confidence: 42,
          supportingEvidence: ['Амьсгал давчдах зовиур'],
          missingEvidence: ['Уушги сонсох үзлэг', 'Цээжний зураг', 'SpO2'],
        },
      ]
    : [
        {
          name: 'Оруулсан шинж тэмдэгт суурилсан ялган оношлогоо',
          confidence: abnormalLabs.length || clinicalCase.symptoms.length || labAttachments.length ? 55 : 25,
          supportingEvidence: [
            ...clinicalCase.symptoms.map((symptom) => symptom.name),
            ...abnormalLabs.map((lab) => `${lab.testName} хэвийн бус`),
            ...labAttachments.map((attachment) => `Лабораторийн зураг хавсаргасан: ${attachment.fileName}`),
          ],
          missingEvidence: [
            'Эмчийн үзлэгийн дэлгэрэнгүй тэмдэглэл',
            ...(labAttachments.length && clinicalCase.labResults.length === 0 ? ['Лаб зургийн OCR/structured review'] : ['Шаардлагатай нэмэлт шинжилгээ']),
          ],
        },
      ]
  ...
  return {
    id: nextId('ai'),
    clinicalSummary: `${clinicalCase.patientName}: ${clinicalCase.chiefComplaint}. Симптом: ${symptomNames}. ${labSummary} ${medicationSummary}`,
    differentialDiagnosis,
    missingInformation: [
      ...(clinicalCase.symptoms.length ? [] : ['Симптомын дэлгэрэнгүй мэдээлэл дутуу']),
      ...(hasLabEvidence ? [] : ['Лабораторийн мэдээлэл дутуу']),
      ...(labAttachments.length && clinicalCase.labResults.length === 0 ? ['Лабораторийн зураг байгаа ч structured lab утга OCR/review хийгдээгүй'] : []),
      ...(activeMedications.length ? [] : ['Идэвхтэй хэрэглэж буй эм бүртгэгдээгүй тул эмийн шалтгаант эрсдэлийг тооцоогүй']),
      ...(chestPain ? ['ЭКГ болон тропонины үр дүн дутуу'] : []),
    ],
    recommendedTests: chestPain
      ? [
          { name: 'ЭКГ', reason: 'Цээжний өвдөлтийн яаралтай үнэлгээ', priority: 'urgent' },
          { name: 'Troponin I/T', reason: 'Зүрхний булчингийн гэмтэл үгүйсгэх', priority: 'urgent' },
          { name: 'SpO2 ба амин үзүүлэлт', reason: 'Амьсгал, гемодинамикийн эрсдэл үнэлэх', priority: 'urgent' },
        ]
      : [
          ...(labAttachments.length && clinicalCase.labResults.length === 0
            ? [{ name: 'Лаб зургийн OCR/review', reason: 'Хавсаргасан зургаас structured lab утга баталгаажуулах', priority: 'routine' as const }]
            : []),
          { name: 'Эмчийн үзлэгийн дэлгэрэнгүй тэмдэглэл', reason: 'AI саналын чанарыг сайжруулах', priority: 'routine' },
        ],
    medicationWarnings,
    causalityAssessment: activeMedications.length
      ? {
          type: medicationWarnings.length ? 'medication_related' : 'unclear',
          confidence: medicationWarnings.length ? 55 : 25,
          evidence: medicationWarnings.length
            ? 'Бүртгэлтэй эм болон лабораторийн өөрчлөлтийг хамтад нь үнэлэх шаардлагатай.'
            : 'Идэвхтэй эм бүртгэлтэй боловч одоогоор тодорхой эмийн шалтгаант high-risk дохио илрээгүй.',
        }
      : {
          type: chestPain ? 'disease_related' : 'unclear',
          confidence: chestPain ? 60 : 20,
          evidence: 'Идэвхтэй хэрэглэж буй эм бүртгэгдээгүй тул эмийн гаж нөлөөний өндөр эрсдэлийн анхааруулга үүсгээгүй.',
        },
    redFlags,
    citations: [
      { title: 'Clinical decision support demo rules', source: 'MedCore local demo', version: '2026', url: '' },
    ],
    confidenceLevel: Math.max(25, Math.min(80, 35 + clinicalCase.symptoms.length * 10 + abnormalLabs.length * 8 + labAttachments.length * 4 + (clinicalCase.hasRedFlag ? 12 : 0))),
